"""
批次圖片優化腳本 — optimize_images.py
======================================

【用途】
把手機拍的大圖（JPG/PNG）批次轉換成適合網頁的 WebP 格式，
大幅減少檔案體積（通常可縮小 50–70%），不影響畫質。

【環境需求】
    pip install Pillow

【使用方式】
方式 A：優化整個 images 資料夾（最常用）
    python tools/optimize_images.py

方式 B：只優化特定資料夾
    python tools/optimize_images.py images/econ-night/2024

【執行後】
- 轉換完成的 .webp 檔會存在原始圖片旁邊
- 原始圖片（.jpg / .png）不會被刪除，確認沒問題後可手動刪除
- 終端機會顯示每張圖的原始大小 → 壓縮後大小與壓縮率

【參數調整（在下方 CONFIG 區修改）】
- MAX_WIDTH：圖片最大寬度（px），超過會自動縮小，建議 1920
- QUALITY：WebP 壓縮品質，0–100，建議 82–88
- SUPPORTED_EXTS：要處理的原始格式
"""

import sys
import os
from pathlib import Path

# ── 參數設定 ──────────────────────────────────────────────────
MAX_WIDTH       = 1920          # 最大寬度（px），超過自動縮小
QUALITY         = 85            # WebP 品質（0–100，85 為品質與體積的甜蜜點）
SUPPORTED_EXTS  = {'.jpg', '.jpeg', '.png', '.heic', '.heif'}
# ─────────────────────────────────────────────────────────────

try:
    from PIL import Image
    # HEIC 需要額外套件，有裝才啟用
    try:
        import pillow_heif
        pillow_heif.register_heif_opener()
    except ImportError:
        pass
except ImportError:
    print("❌  找不到 Pillow，請先安裝：pip install Pillow")
    sys.exit(1)


def human_size(size_bytes: int) -> str:
    """將 bytes 轉成可讀字串"""
    if size_bytes < 1024:
        return f"{size_bytes} B"
    elif size_bytes < 1024 ** 2:
        return f"{size_bytes / 1024:.1f} KB"
    else:
        return f"{size_bytes / 1024 ** 2:.1f} MB"


def convert_image(src: Path) -> None:
    """將單張圖片轉換為 WebP"""
    dst = src.with_suffix('.webp')

    # 若 webp 已存在且比原檔新，跳過
    if dst.exists() and dst.stat().st_mtime >= src.stat().st_mtime:
        print(f"  ⏭  略過（已是最新）：{src.name}")
        return

    with Image.open(src) as img:
        # 修正 EXIF 旋轉（手機照片常見問題）
        try:
            from PIL import ImageOps
            img = ImageOps.exif_transpose(img)
        except Exception:
            pass

        # 轉換色彩模式（PNG 可能是 RGBA）
        if img.mode in ('RGBA', 'P'):
            img = img.convert('RGBA')
        else:
            img = img.convert('RGB')

        # 縮小超寬圖片
        if img.width > MAX_WIDTH:
            ratio = MAX_WIDTH / img.width
            new_h = int(img.height * ratio)
            img = img.resize((MAX_WIDTH, new_h), Image.LANCZOS)

        img.save(dst, 'WEBP', quality=QUALITY, method=6)

    orig_size = src.stat().st_size
    new_size  = dst.stat().st_size
    saving    = (1 - new_size / orig_size) * 100
    print(f"  ✅  {src.name} → {dst.name}  "
          f"({human_size(orig_size)} → {human_size(new_size)}, 節省 {saving:.0f}%)")


def process_folder(folder: Path) -> None:
    """遞迴處理資料夾內所有支援格式的圖片"""
    images = [
        p for p in sorted(folder.rglob('*'))
        if p.is_file() and p.suffix.lower() in SUPPORTED_EXTS
    ]

    if not images:
        print(f"📂  {folder}：沒有找到需要轉換的圖片")
        return

    print(f"\n📂  處理資料夾：{folder}（共 {len(images)} 張）")
    for img_path in images:
        try:
            convert_image(img_path)
        except Exception as e:
            print(f"  ❌  {img_path.name} 轉換失敗：{e}")


if __name__ == '__main__':
    # 從腳本所在位置往上找 images 資料夾
    script_dir  = Path(__file__).parent
    project_dir = script_dir.parent

    if len(sys.argv) > 1:
        # 使用者指定資料夾
        target = Path(sys.argv[1])
        if not target.is_absolute():
            target = project_dir / target
    else:
        # 預設處理整個 images/ 資料夾
        target = project_dir / 'images'

    if not target.exists():
        print(f"❌  找不到資料夾：{target}")
        sys.exit(1)

    print("=" * 55)
    print("  圖片優化腳本（→ WebP）")
    print(f"  目標：{target}")
    print(f"  最大寬度：{MAX_WIDTH}px，品質：{QUALITY}")
    print("=" * 55)

    process_folder(target)

    print("\n✨  完成！請確認 .webp 檔案後，再手動刪除原始圖片。")
    print("    完成後記得更新 assets/js/gallery-data.js 填入路徑。")
